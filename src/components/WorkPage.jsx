      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-gradient-to-br from-cyan-900/30 to-cyan-600/10 backdrop-blur-xl border border-cyan-500/30 rounded-2xl p-5 flex items-center justify-between shadow-[0_0_20px_rgba(0,243,255,0.05)]">
            <div><p className="text-xs text-cyan-300/70">{t.vipLevel}</p><h3 className="text-2xl font-bold text-cyan-400">VIP {vipLevel}</h3></div>
          ...
          <div className="bg-gradient-to-br from-green-900/30 to-green-600/10 backdrop-blur-xl border border-green-500/30 rounded-2xl p-5 flex items-center justify-between shadow-[0_0_20px_rgba(34,197,94,0.05)]">
            <div><p className="text-xs text-green-300/70">{t.dailyCommission}</p><h3 className="text-2xl font-bold text-green-400">{currentRate}%</h3><p className="text-xs text-green-300/50">${totalProfit.toFixed(2)}</p></div>
          ...
          <div className="bg-gradient-to-br from-yellow-900/30 to-yellow-600/10 backdrop-blur-xl border border-yellow-500/30 rounded-2xl p-5 flex items-center justify-between shadow-[0_0_20px_rgba(250,204,21,0.05)]">
            <div><p className="text-xs text-yellow-300/70">{t.completedTasks}</p><h3 className="text-2xl font-bold text-yellow-300">{completedCount} / 5</h3></div>
          ...
          <div className="bg-gradient-to-br from-purple-900/30 to-purple-600/10 backdrop-blur-xl border border-purple-500/30 rounded-2xl p-5 flex items-center justify-between shadow-[0_0_20px_rgba(168,85,247,0.05)]">
            <div><p className="text-xs text-purple-300/70">{t.resetTime}</p><h3 className="text-xl font-mono font-bold text-purple-300">{timeToReset}</h3></div>
        ...
        {allTasksDone ? (
          <div className="bg-gradient-to-br from-green-500/20 via-cyan-500/10 to-blue-500/20 border border-green-500/40 rounded-3xl p-10 text-center shadow-[0_0_50px_rgba(34,197,94,0.15)] backdrop-blur-2xl">
            ...
            <p className="text-xs text-gray-400 mt-6 border-t border-white/10 pt-4">{t.contractLocked} (${lockedAmount.toFixed(2)})</p>
          </div>
        ) : (
          <>
            {contractAmount === 0 || vipLevel === 0 ? (
              <div className="text-center py-16 bg-white/5 rounded-3xl border border-white/10 backdrop-blur-sm">
                ...
                <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="mt-6 px-8 py-3 rounded-2xl bg-gradient-to-r from-cyan-500 to-[#00f3ff] text-slate-950 font-bold shadow-[0_0_25px_rgba(0,243,255,0.4)] hover:shadow-[0_0_40px_rgba(0,243,255,0.7)] transition-all">
                  ...
                  return (
                    <div key={idx} className={`group relative overflow-hidden transition-all duration-500 backdrop-blur-xl rounded-2xl p-6 border flex flex-col md:flex-row items-center justify-between gap-4 ${isCompleted ? 'bg-gradient-to-r from-green-500/20 to-emerald-500/10 border-green-500/40 shadow-[0_0_25px_rgba(34,197,94,0.1)]' : isCurrent ? 'bg-gradient-to-r from-cyan-500/30 to-[#00f3ff]/10 border-[#00f3ff] shadow-[0_0_35px_rgba(0,243,255,0.25)] animate-pulse' : 'bg-white/5 border-white/10 opacity-60 hover:opacity-100'}`}>
                      ...
                      <div className="flex items-center gap-3">
                        {isCompleted ? (
                          <div className="flex items-center gap-2 text-green-400 font-bold px-5 py-2.5 rounded-xl bg-green-500/10 border border-green-500/30 backdrop-blur-sm"><CheckCircle2 className="w-5 h-5 animate-pulse" /> <span className="text-sm">مكتملة</span></div>
                        ) : isLocked ? (
                          <div className="flex items-center gap-2 text-gray-500 px-5 py-2.5 rounded-xl bg-white/5 border border-white/10"><Lock className="w-5 h-5" /> <span className="text-sm">مقفلة</span></div>
                        ) : (
                          <button onClick={() => handleExecuteTask(idx)} disabled={isProcessing} className="relative overflow-hidden px-8 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-[#00f3ff] text-slate-950 font-black shadow-[0_0_25px_rgba(0,243,255,0.4)] hover:scale-105 transition-all duration-300 disabled:opacity-50 flex items-center gap-2 group">
                            <span className="absolute inset-0 w-full h-full bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700"></span>
                            {isProcessing && activeTaskIndex === idx ? (
                              <><RefreshCw className="w-5 h-5 animate-spin" /> <span>{t.processing}</span></>
                            ) : (
                              <><Play className="w-5 h-5 fill-slate-950" /> <span>{t.startTask}</span></>
                            )}
                          </button>
                        )}
                      </div>
