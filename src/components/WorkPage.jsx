    ...
    <div className="min-h-screen bg-[#030914] text-white p-4 md:p-8 font-sans" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        {showRenewMessage && (
          <div className="bg-gradient-to-r from-green-500/20 to-cyan-500/20 border border-green-500/50 rounded-3xl p-6 text-center">
            ...
            <p className="text-cyan-200">
              {t.profitAdded}: <span className="text-green-400 font-extrabold">${totalProfit.toFixed(2)}</span>
            </p>
            <p className="text-xs text-gray-400">{t.contractLocked} (${lockedAmount.toFixed(2)})</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-[#00f3ff]/[0.05] backdrop-blur-xl border border-[#00f3ff]/30 rounded-3xl p-6 flex justify-between">
            <div>
              <p className="text-xs text-cyan-300/70">{t.dailyCommission}</p>
              <h3 className="text-2xl font-bold text-green-400">{currentRate}% (${totalProfit.toFixed(2)})</h3>
            </div>
            <DollarSign className="w-10 h-10 text-green-400" />
          </div>
          <div className="bg-[#00f3ff]/[0.05] backdrop-blur-xl border border-[#00f3ff]/30 rounded-3xl p-6 flex justify-between">
            <div>
              <p className="text-xs text-cyan-300/70">{t.resetTime}</p>
              <h3 className="text-xl font-mono font-bold text-yellow-300">{timeToReset}</h3>
            </div>
            <RefreshCw className="w-9 h-9 text-yellow-300 animate-spin-slow" />
          </div>
        </div>
        ...
        <div className="bg-[#00f3ff]/[0.05] backdrop-blur-xl border border-[#00f3ff]/20 rounded-2xl p-4">
          ...
          <div className="w-full h-3 bg-slate-900 rounded-full overflow-hidden border border-[#00f3ff]/20 p-0.5">
            <div
              className="h-full bg-gradient-to-r from-cyan-500 to-[#00f3ff] rounded-full transition-all duration-500"
