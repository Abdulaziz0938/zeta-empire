...
const AdminPanel = ({ onBack, onNavigate }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (usersData.success) {
        ...
        // في حال فشل الجلب، استخدم بيانات وهمية للتجربة
        setUsers([
      ...
      // جلب المعاملات
      const txsRes = await fetch(`${import.meta.env.VITE_API_URL}/api/transactions`);
      ...
      if (txsData.success) {
        ...
        setTransactions([
          ...
          { id: 'TX-1091', userName: 'سارة خالد', phone: '+966559876543', type: 'withdraw', amount: 150, network: 'BEP20', txHash: '-', address: '0x71C7...8976F', date: '2026-09-03 10:15', status: 'pending', adminAction: null, note: '' },
        ]);
      }
  ...
  // ===== دوال الإجراءات (تعمل على البيانات المحلية) =====
      ...
      <div className="max-w-7xl mx-auto space-y-6 relative">
        ...
        <div className={`${cardBg} backdrop-blur-2xl border ${borderColor} rounded-3xl p-6 shadow-[0_0_30px_rgba(0,243,255,0.1)] flex flex-col md:flex-row items-center justify-between gap-4 transition-colors duration-300`}>
          ...
          <div className="flex items-center gap-3 flex-wrap justify-center">
            ...
            <button onClick={() => alert('سيتم إرسال إشعار جماعي قريباً')} className="px-4 py-2 rounded-2xl bg-gradient-to-r from-cyan-500 to-[#00f3ff] text-slate-950 font-bold text-xs shadow-[0_0_15px_rgba(0,243,255,0.3)] hover:shadow-[0_0_25px_rgba(0,243,255,0.6)] transition-all flex items-center gap-2">
                        ...
                        <td className="p-3 text-center">
                          {tx.status === 'pending' ? (
                            <div className="flex items-center justify-center gap-2">
                              ...
                              <button onClick={() => {}} className="p-1.5 rounded-lg bg-cyan-500/20 hover:bg-cyan-500 text-cyan-400 hover:text-white border border-cyan-500/30 transition-all" title="تفاصيل"><Eye className="w-4 h-4" /></button>
                            </div>
                          ) : (
                            <span className="text-gray-500 text-[10px]">تم المعالجة</span>
                          )}
                        </td>
                      </tr>
                    ))
                      ...
                      <tr key={u.id} className="hover:bg-white/5 transition-colors">
                        ...
                        <td className="p-3">
                          <div className="flex items-center justify-center gap-1.5">
                            <button onClick={() => alert(`إرسال إشعار إلى ${u.fullName}`)} className="p-1.5 rounded-lg bg-cyan-500/20 hover:bg-cyan-500 text-cyan-400 hover:text-white border border-cyan-500/30 transition-all" title="إرسال إشعار مخصص"><MessageSquare className="w-4 h-4" /></button>
                            <button onClick={() => alert(`تحرير رصيد ${u.fullName}`)} className="p-1.5 rounded-lg bg-yellow-500/20 hover:bg-yellow-500 text-yellow-400 hover:text-white border border-yellow-500/30 transition-all" title="تحرير الرصيد"><Edit3 className="w-4 h-4" /></button>
